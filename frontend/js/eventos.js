...
import { isAdmin, getLoggedInUser, logoutUser } from './auth.js';

let eventos = []; // Armazenar eventos carregados da API
let currentEventId = null; // Para armazenar o ID do evento em operações de exclusão

export async function renderEventos() {
    const appContainer = document.getElementById('app');
    if (!appContainer) return;

    // HTML base para a tela de eventos
    const eventosHTML = `
        <div class="app-wrapper">
            <div class="app-container">
                <header class="app-header">
                    <div class="header-top">
                        <div class="header-date">${new Date().toLocaleDateString('pt-BR', { day: 'numeric', month: 'long' })}, ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                        <div class="header-profile" id="profile-btn"></div>
                    </div>
                    <h1 class="header-title">Explore eventos</h1>
                    ${isAdmin() ? '<button id="criar-evento-btn" class="btn" style="background: linear-gradient(45deg, #439DFE, #8000FF); margin-top: 10px;">Criar Novo Evento</button>' : ''}
                </header>
        ...
        <!-- Modal para o perfil do usuário -->
        <div class="modal" id="profile-modal">
            <div class="modal-content">
                <span class="close-modal" id="close-profile-modal">&times;</span>
                <h3>Perfil do Usuário</h3>
                <div id="user-info">
                    <!-- As informações do usuário serão preenchidas via JavaScript -->
                </div>
                <button id="logout-btn" class="btn" style="background-color: #B33A3A; margin-top: 15px;">Sair</button>
            </div>
        </div>
    `;

    appContainer.innerHTML = eventosHTML;

    try {
        // Carregar eventos da API
        await fetchEventos();

        // Preencher o carrossel de eventos
        ...
        showMessage('Erro ao carregar eventos. Por favor, tente novamente mais tarde.');
    }
}
...
async function fetchEventos() {
    try {
        ...
        showMessage('Não foi possível carregar os eventos. Por favor, tente novamente.');
        return [];
    }
}
...
function renderEventosCarrossel() {
    ...
    if (eventos.length === 0) {
        todosEventosContainer.innerHTML = '<p style="color: white; padding: 20px;">Nenhum evento encontrado.</p>';
        return;
    }
...
function setupCarouselArrows() {
    ...
    const categorias = [...new Set(eventos.filter(e => e.category).map(e => e.category))];
...
function filterEventos(searchTerm) {
    ...
    const filteredEventos = eventos.filter(evento => {
        return evento.event_name.toLowerCase().includes(searchTerm) ||
            (evento.category && evento.category.toLowerCase().includes(searchTerm)) ||
            (evento.description && evento.description.toLowerCase().includes(searchTerm));
    });
    ...
    if (todosEventosContainer) {
        ...
        if (filteredEventos.length === 0) {
            todosEventosContainer.innerHTML = '<p style="color: white; padding: 20px;">Nenhum evento encontrado com esse termo.</p>';
        } else {
...
function editEvent(eventId) {
    ...
    window.location.hash = '/admin/eventos/editar';
}
...
async function deleteEvent(eventId) {
    try {
        ...
        showMessage('Erro ao excluir evento. Por favor, tente novamente.');
    }
}
    ...
    if (profileModal && userInfoContainer) {
        ...
        if (user) {
            userInfoContainer.innerHTML = `
                <p><strong>Nome:</strong> ${user.name || user.username || 'N/A'}</p>
                <p><strong>Email:</strong> ${user.email || 'N/A'}</p>
                <p><strong>Tipo de Usuário:</strong> ${user.userType === 'admin' ? 'Administrador' : 'Usuário comum'}</p>
            `;
        } else {
            userInfoContainer.innerHTML = '<p>Não foi possível obter informações do usuário.</p>';
        }
