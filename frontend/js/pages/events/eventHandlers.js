...
export function setupUIEvents() {
  ...
  // Configuração especial para as setas do carrossel de eventos filtrados (caso existam)
  ...
  if (filteredLeftArrow && filteredRightArrow && filteredCarousel) {
    // Configurar seta esquerda
    const newLeftArrow = filteredLeftArrow.cloneNode(true);
    filteredLeftArrow.parentNode.replaceChild(newLeftArrow, filteredLeftArrow);
    newLeftArrow.addEventListener('click', () => {
      if (newLeftArrow.classList.contains('is-visible')) {
        const scrollAmount = filteredCarousel.clientWidth * 0.8;
        filteredCarousel.scrollBy({ left: -scrollAmount, behavior: 'smooth' });
      }
    });

    // Configurar seta direita
    const newRightArrow = filteredRightArrow.cloneNode(true);
    filteredRightArrow.parentNode.replaceChild(newRightArrow, filteredRightArrow);
    newRightArrow.addEventListener('click', () => {
      if (newRightArrow.classList.contains('is-visible')) {
        const scrollAmount = filteredCarousel.clientWidth * 0.8;
        filteredCarousel.scrollBy({ left: scrollAmount, behavior: 'smooth' });
      }
    });
    ...
    updateCarouselArrowVisibility(filteredCarousel);
  }
