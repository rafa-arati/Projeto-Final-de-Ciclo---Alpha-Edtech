...
\
        ...
        <div id="qrcode-validation-buttons" class="qrcode-buttons hidden">
          <button id="retry-scan-btn" class="btn btn-primary">Escanear Novamente</button>
    ...
    function scanQRCode() {
      ...
      try {
        ...
        if (code) {
          ...
          stopCamera();
          // Mudança: Automaticamente validar o QR code após detectar
 ...
 */
async function validateQRCodeAPI(qrCode) {
  if (!qrCode) {
  ...
  if (!resultContainer || !buttonsContainer) return;

  try {
    // Primeiro, verificamos novamente os detalhes do QR Code para garantir a validação do evento
    const detailsResponse = await fetch(`/api/qrcode/validate/${encodeURIComponent(qrCode)}`);
    ...
    const response = await fetch(`/api/qrcode/use/${encodeURIComponent(qrCode)}`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok && data.success !== false) {
      ...
      // Mostrar botão para escanear novamente
    ...
    // Mostrar botão para escanear novamente mesmo em caso de erro
    buttonsContainer.classList.remove('hidden');
    window.currentQRCode = null;
  }
}
  ...
  try {
    ...
    if (response.ok && data.isValid) {
      ...
      if (currentEventName && eventName && currentEventName !== eventName) {
        ...
        // Mostrar botão para escanear novamente mesmo quando é evento errado
        buttonsContainer.classList.remove('hidden');
        window.currentQRCode = null;
        return;
      }
      ...
      // MUDANÇA: Validação automática do QR Code após verificação
      setTimeout(() => {
        validateQRCodeAPI(qrValue);
      }, 500); // Pequeno atraso para melhor experiência do usuário
    } else {
...
export async function setupQRCodeValidator(eventIdParam) {
  ...
  if (!cancelButton || !resultContainer || !buttonsContainer) {
  ...
  // Configurar botão para escanear novamente
  if (retryScanBtn) {
    const newRetryScanBtn = retryScanBtn.cloneNode(true);
    retryScanBtn.parentNode.replaceChild(newRetryScanBtn, retryScanBtn);
    newRetryScanBtn.addEventListener('click', () => {
      // Limpar resultado e esconder botões
      resultContainer.classList.add('hidden');
      buttonsContainer.classList.add('hidden');

      // Iniciar câmera novamente
      const startScanBtn = document.getElementById('start-scan-btn');
      if (startScanBtn) {
        startScanBtn.click();
      }
