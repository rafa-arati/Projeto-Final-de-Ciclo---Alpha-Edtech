  ...
  if (profileForm) {
    ...
    newProfileForm.addEventListener('submit', async (e) => {
      ...
      if (rawPhone && (rawPhone.length < 10 || rawPhone.length > 11)) {
        showInlineFeedback('Telefone inválido (10 ou 11 dígitos).', 'error', feedbackElementId); // <- Usa a função local
        return;
      }
      if (birthDateValue) { // Valida data futura
        try {
          ...
          if (birthDateObj > today) {
            showInlineFeedback('Insira uma data de nascimento válida.', 'error', feedbackElementId); // <- Usa a função local
            return;
          }
        } catch (e) {
          showInlineFeedback('Data de nascimento inválida.', 'error', feedbackElementId); // <- Usa a função local
          return;
        }
      }
