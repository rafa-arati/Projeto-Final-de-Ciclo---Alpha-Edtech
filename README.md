...
Projeto final de ciclo desenvolvido para a Alpha Edtech, consistindo em uma aplicação web full-stack para descoberta e gerenciamento de eventos culturais, com funcionalidades administrativas e sistema de QR Code para promoções.
...
O **Rota Cultural** foi desenvolvido como uma plataforma centralizada para conectar pessoas a eventos culturais em sua localidade, ao mesmo tempo que oferece ferramentas para que administradores possam criar, divulgar e gerenciar eventos de forma eficiente.

**Problema:** Muitas vezes, a descoberta de eventos culturais locais é fragmentada, e organizadores carecem de ferramentas acessíveis para gerenciar promoções, controlar acesso e divulgar suas atividades.
...
* Criação e gerenciamento de eventos para administradores.
* Sistema de autenticação seguro (Email/Senha e Google OAuth).
* Funcionalidade de "curtidas" para engajamento.
* Um sistema integrado de promoções via QR Code, permitindo que administradores ofereçam benefícios e validem a participação.
...
* **Administradores:** Têm controle total sobre a plataforma, gerenciando eventos, usuários e promoções.
...
Este projeto visa não apenas organizar a informação cultural, mas também fomentar a participação e facilitar a gestão de eventos.
...
* **Autenticação:**
    ...
    * Sistema de Papéis (User, Admin).
* **Eventos:**
    * Criação, Leitura, Atualização e Exclusão (CRUD) por Administradores.
...
* **Promoções e QR Code:**
    * Criação de promoções vinculadas a eventos (Desconto, Brinde, VIP, etc.) por Administradores.
    * Definição de limites (quantidade, prazo de geração, prazo de uso).
    * Geração de QR Code único por usuário para uma promoção.
    * Validação de QR Codes (via câmera ou manual) por Administradores para marcar como usado.
...
2.  **Variáveis de Ambiente:**
    ...
    * Adicione as seguintes variáveis, preenchendo com suas credenciais e configurações:
        ```env
...
* **Administrador (`admin`):** Todas as funcionalidades do usuário comum, mais a capacidade de criar, editar e excluir *qualquer* evento, criar e gerenciar promoções (QR Codes) para eventos e validar QR Codes.

*Nota: O projeto não cria usuários padrão automaticamente. Você precisará se registrar.*
...
https://equipe01.alphaedtech.org.br
